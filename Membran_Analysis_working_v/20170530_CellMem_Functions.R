...
dyMaxHistogram_ah<-function(mydata, low = 0, up = 0, c = 2, set_x_marker = 0, dymax_low = 0, dymax_up = 0, bi = 500, 
                         pr = 0, w =5, h = 3, res = 200, myname = "sys.time",  colS = colSet){
      plots<-list()
      n=0
      mindy = 0
      maxdy = 0
      ...
      for (i in unique(myfdata$f_on_samp)){ 
            mysummary_data <- myfdata[myfdata$f_on_samp==i,]
            mysummarydf[1,p] <- round(mean(mysummary_data$dymax),4)
            mysummarydf[2,p] <- round(sd(mysummary_data$dymax),4)
            p <- p + 1
      }      
      p <- 0
      ...
      grid.newpage()
      pushViewport(viewport(layout=grid.layout(3,length(unique(myfdata$f_on_samp)))))
      print(g1, vp=viewport(layout.pos.row = 1:2, layout.pos.col = 1:length(unique(myfdata$f_on_samp))))
      ...
      for (i in unique(myfdata_subset$f_on_samp)){
            ...
            number_of_intensities <-3 ###Determined in peakfinder function
            peaks_system <- c(c(rep(c(j),number_of_intensities)),peaks_system)
            }
            
            peaks_each_system <- cbind(peaks_system,peaks)
            colnames(peaks_each_system) <- c("System", "dymax")
            peaks_each_system <- as.data.frame(peaks_each_system)
            ...
            g2 <- ggplot() + theme_void() + xlim(0, 1) + ylim(0, 1) + 
                  geom_vline(data = peaks_each_system, aes(xintercept = as.numeric(dymax))) +
                  annotation_custom(tableGrob(as.matrix(mysummaryf)), xmin=-0.5, ymin=0.2) 

            
            print(g2, vp=viewport(layout.pos.row = 3, layout.pos.col = p))
            p <- p +1
            mysummary_complete <- NULL
            peaks <- NULL
            peaks_system <- NULL
