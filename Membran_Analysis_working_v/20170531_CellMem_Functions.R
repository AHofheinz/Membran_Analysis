...
Ingestion <- function(alpha, ColumnsOfInterest, fileLineOffset, mypath, lowlimit, uplimit){
        ...
        for (i in allfiles){
                ...
                ####extracting infos from pathname
                fnc2<-strsplit(myfile, "\\.")
                for (i in 1:length(fnc2[[1]])){
                      if (fnc2[[1]][i]=="Load") {LS <- "Load"}
                      if (fnc2[[1]][i]=="Speed") {LS <- "Speed"}
                      else {LS <- "NA"}
                }
...
PreprocessingIngestedRawData<-function(myfilename, y_trace__y_retrace = "y_trace"){
        setwd("C:/Users/User/Documents/UIUC/20170518_Membran")
        mypath<-""
        getwd()
        df<-read.csv(paste0(mypath, myfilename))[, c("x", y_trace__y_retrace,"loop", "f_on_samp", "system", "ID", "line_rate", "length", "tip", "alpha", "date", "LS")]
        #df<-read.xlsx("df_jump.xlsx", 2)
        mydata<-df
        # columnname loop changes from loop to half_loop, because only tracing or retracing is read. 
        colnames(mydata)<-c("x", "y", "half_loop", "f_on_samp", "system", "ID", "line_rate", "length", "tip", "alpha", "date", "LS")
...
MinMaxSlope <- function(mydata){
        ...
        mydata <- mydata %>% select(x:y, dxmax, dymax, slope, reg.slope, reg.intercept,  xmin:ymax, x.fitted, y.resid, x.fitted.min, x.fitted.max, 
                                    ...
                                    system, trace_d, mSL, line_rate, length, tip, date, alpha, LS)
...
#####################################################################################################
#####################################################################################################
dyMaxHistogram_ah<-function(mydata, low = 0, up = 0, c = 2, set_x_marker = 0, dymax_low = 0, dymax_up = 0, bi = 500, 
                            pr = 0, w =5, h = 3, res = 200, myname = "sys.time",  colS = colSet, peak_analysis = FALSE){
      ...
      mincount = 0
      maxcount = 0
      mysummarydf <- data.frame()
      mysummary <- data_frame()
      mysummary_complete <- NULL
      mysummary_names <- NULL
      
      if (low == 0 & up == 0 ) {
            low<-min(mydata$slope)
            up<-max(mydata$slope)}
      
      if (dymax_low == 0 & dymax_up == 0 ) {
            dymax_low<-min(mydata$dymax)
            dymax_up<-max(mydata$dymax)}
      
      if (peak_analysis == TRUE){
            peaks <- NULL
            peaks_system <- NULL
            peaks_force <- NULL
            peaks_each_force_system <- NULL
            peaks_each_force_system_complete <- list()
            
      }
      
      #Cleaning the data by using unique dymax values only
      myfdata<-mydata[mydata$dymax!=0,]
      myfdata<-myfdata[!duplicated(myfdata$cluster), ]
      myfdata<-myfdata[order(myfdata$system),]
      myfdata$f_on_samp <- factor(myfdata$f_on_samp, levels=unique(myfdata$f_on_samp))
      
      
      
      #Subsetting the data by slope and dymax
      myfdata_subset <- myfdata[(myfdata$slope>low & myfdata$slope<up), ]
      myfdata_subset <- myfdata_subset[(myfdata_subset$dymax>dymax_low & myfdata_subset$dymax<dymax_up),]
      
      #For avoiding of e-04 in summary tables
      options(scipen = 10)
      
      #Determine the layout of the grid (Poisitions of the tables)
      library(grid)
      grid.newpage()
      pushViewport(viewport(layout=grid.layout(5,length(unique(myfdata$f_on_samp)))))
      
      #Determines the layout for the summary tables
      tt2 <- ttheme_minimal()
      ...
      #For all forces
      for (i in unique(myfdata_subset$f_on_samp)){
            #For all systems
            for (j in unique(myfdata_subset$system)){
                  
                  #Generating summary for every system for every force      
                  mysummary <- data.frame(summary(myfdata_subset[myfdata_subset$f_on_samp==i & myfdata_subset$system==j,"dymax"]))
                  mysummary_split <- data.matrix(do.call('rbind', strsplit(as.character(mysummary[,3]),':',fixed=TRUE)))
                  mysummary_split <- mysummary_split[,2]
                  stand_dev <- round(sd(myfdata_subset[myfdata_subset$f_on_samp==i & myfdata_subset$system==j,"dymax"][[1]]),5)
                  mysummary_split <- c(mysummary_split, stand_dev)
                  
                  #Combining separate summaries
                  mysummary_complete <- cbind(mysummary_complete, mysummary_split)
                  mysummary_names <- c(j,mysummary_names)
                  
                  if (peak_analysis == TRUE){
                        peaksfound <- peakfinder_ah(myfdata_subset[myfdata_subset$f_on_samp==i & myfdata_subset$system==j,"dymax"][[1]])
                        peaks <- c(peaksfound, peaks)
                        number_of_intensities <-2 ###Determined in peakfinder function
                        peaks_system <- c(c(rep(c(j),number_of_intensities)),peaks_system)
                        peaks_force <- c(c(rep(c(i),number_of_intensities)), peaks_force)
                  }
            }
            
            if (peak_analysis == TRUE){
                  peaks_each_force_system <- as.data.frame(cbind(peaks_force,peaks_system,as.numeric(peaks)))
                  #peaks_each_force_system_complete <- rbind(peaks_each_force_system, peaks_each_force_system_complete)
                  colnames(peaks_each_force_system) <- c("f_on_samp" ,"system", "dymax")
                  peaks_each_force_system_complete[[p]] <- as.data.frame(peaks_each_force_system)
            }
            
            #Assigning names to the summary variables
            category_vector <- c("Minimum:","1st Quantile:","Median:","Mean:","3rd Quantile:","Maximum:", "SD")
            mysummaryf<- cbind(category_vector,mysummary_complete)
            colnames(mysummaryf) <- c("Summary", mysummary_names)
            
            #Saving and plotting of the summary tables 
            g2 <- ggplot() + theme_void() + xlim(0, 1) + ylim(0, 1) + 
                  annotation_custom(tableGrob(as.matrix(mysummaryf)), xmin=-0.2, ymin=0.2)
            print(g2, vp=viewport(layout.pos.row = 4:5, layout.pos.col = p))
            
            p <- p +1
            
            #Clear variables for next force
            mysummary_complete <- NULL
            mysummary_names <- NULL
            ...
            if (peak_analysis == TRUE){
                  peaks <- NULL
                  peaks_system <- NULL
                  peaks_each_system <- NULL
                  peaks_each_force_system <- NULL
                  peaks_force <- NULL
            }
      }
      
      #Saving and plotting of the histograms
      g1 <- ggplot() + geom_histogram(data=myfdata_subset, aes(x=dymax, fill = system), bins = bi) +
            facet_grid(.~f_on_samp)
      
      z <- 1
      
      if (peak_analysis == TRUE){
            peaks_all_force <- do.call(rbind, peaks_each_force_system_complete)
            
            #for (i in unique(myfdata_subset$f_on_samp)){
            for (j in unique(peaks_all_force$system)){
                  
            peaks_all_force_sub <-   myfdata_subset[peaks_all_force$system==j,]
            mymean <-   peaks_all_force_sub$dymax 
            mysd <- sd(myfdata_subset[myfdata_subset$system ==j, "dymax"][[1]])

            colo <- myColors[z]
            g1 <- g1 + geom_vline(data = peaks_all_force_sub, 
                                  aes(xintercept = as.numeric(as.character(dymax))), color = colo, size = 0.05) + 
                  stat_function( 
                        fun = function(x, mean, sd, n, bw){ 
                              dnorm(x = x, mean = mymean, sd = mysd)
                        }, 
                        args = c(mean = mymean, sd = mysd)
                  
                 # stat_function(fun = dnorm, args(list( mean = mymean, sd = mysd)))
            
            z <- z +1
            #}
      }
      }      
      print(g1, vp=viewport(layout.pos.row = 1:3, layout.pos.col = 1:length(unique(myfdata$f_on_samp))))
      as.numeric(as.character(myfdata_subset$dymax))
}


###########################################################################################################################
###########################################################################################################################
peakfinder_ah <- function(d){
      dh <- hist(d,plot=FALSE, breaks = 20)
      ins <- dh[["counts"]]
      nbins <- length(ins)
      #nbins <- 500
      ss <- which(rank(ins)%in%seq(from=nbins-1,to=nbins)) ## pick the top 2 intensities
      dh[["mids"]][ss]
}
