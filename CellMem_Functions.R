...
PreprocessingIngestedRawData<-function(myfilename, y_trace__y_retrace = "y_trace"){
        setwd("C:/Users/User/Documents/UIUC/20170518_Membran/FrictionCellMembranes/FrictionCellMembranes")
        mypath<-""
        getwd()
...
#####################################################################################################
#####################################################################################################
dyMaxHistogram<-function(mydata, low = 0, up = 0, c = 2, set_x_marker = 0,  same_scale = FALSE, logscale = FALSE, bi = 500, 
      ...
      mysummarymatrix <- data.matrix(mysummarydf) 
      rownames(mysummarymatrix) <- c("mean","sd")
      mysummarymatrix <- as.data.frame(mysummarymatrix)
      

      
      mysummary <- mytable
      names(mysummary) <- c("","Value")
      tt2 <- ttheme_minimal()
      
      g1 <- ggplot() + geom_histogram(data=myfdata[(myfdata$slope>low & myfdata$slope<up), ], aes(x=dymax, fill = system), bins = bi) +
